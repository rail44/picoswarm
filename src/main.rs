...
fn main() -> anyhow::Result<()> {
    ...
    match parsed.command {
        ...
        cli::Command::Run {
            ...
            init_client_tracing();
            client_runtime().block_on(client::run::run(name, detach, spawn, cmd))
        }
        ...
        cli::Command::Event { event } => {
            init_client_tracing();
            client_runtime().block_on(client::event::run(event))
        }
        cli::Command::Register { name } => {
            init_client_tracing();
            client_runtime().block_on(client::register::run(name))
        }
